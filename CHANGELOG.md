...
### Documentation
- **Single handbook:** Technical documentation merged into **[`docs/HANDBOOK.md`](docs/HANDBOOK.md)**. Removed split files (`ARCHITECTURE`, `ENGINE_PIPELINE`, `FILE_FORMATS`, `FUNCTION_REFERENCE`, `MAINTENANCE`, `VISUAL_STYLING`, `CLIFF_TREE_IMPLEMENTATION`, `Roadmap/*`, root `GUIDE.md`). **`README.md`** now points to the handbook; **RX Converter** CLI remains documented in **`tools/RXConverter/README.md`**.
- **Wang layouts:** Restored full **[`docs/wang_tile_layouts.md`](docs/wang_tile_layouts.md)** (1×13 strip, 5×3 / 5×4 grids, master semantic table, `MG_WANG_SEMANTIC_TO_GRID`, `MG_CLIFF_DOUBLE_CD_MAP`); §2 row/corner labels corrected to match **`mgLayers.js`**. **`README.md`** / handbook link to it as the detailed reference.
...
- **Scripts:** `npm run test` (Vitest), `npm run lint` (ESLint), `npm run format` (Prettier).
