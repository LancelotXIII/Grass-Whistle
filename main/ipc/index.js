...
const { scanGameFolder } = require('../services/scanGameFolder')
...
function registerIpcHandlers({ appRoot }) {
  ...
  ipcMain.handle('scan-game-folder', async () => {
    try {
      const result = await dialog.showOpenDialog({ properties: ['openDirectory'], title: 'Select your RPG Maker XP game folder' })
      if (result.canceled || !result.filePaths[0]) return { ok: false, canceled: true }
      return await scanGameFolder({ gameDir: result.filePaths[0], appRoot })
    } catch (err) {
      console.error('scan-game-folder failed:', err)
