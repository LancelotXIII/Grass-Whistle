...
function buildRmxpExportReferenceMarkdown(opts) {
  ...
  if (tilesetOnly) {
    ...
    lines.push('')
    lines.push(
      'Each group gets a **1×1 blank parent map** (folder in the RPG Maker map tree) and one **child map per panel**, named `{Group name} A`, `B`, … in top-left → bottom-right order within the group.',
    )
    lines.push('')
    ...
    for (const w of written) {
      const role = w.rmxpRole === 'group_parent' ? 'group parent (blank)' : 'panel'
      const pid = w.parentMapId != null && (w.parentMapId | 0) > 0 ? String(w.parentMapId | 0) : '—'
      const xy =
        w.rmxpRole === 'group_parent' || w.x == null || w.y == null ? '—' : `(${w.x},${w.y})`
      lines.push(
