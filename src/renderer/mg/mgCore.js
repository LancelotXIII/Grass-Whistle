...
/**
 * Cliff tileIndex encodes which direction the edge drops toward (see layoutGen Step 11).
 * Returns the approximate downhill neighbor offset.
 * @param {number} ti
 * @returns {[number, number]}
 */
function cliffDownhillOffset(ti) {
...
function mgLayerBit(layer, t) {
  if (!t) return false
  if (layer === 'land') {
    return t === T.LAND || t === T.FOREST || t === T.ROAD || t === T.CLIFF || t === T.GRASS
  }
...
/**
 * Loads the eight orthogonally and diagonally adjacent panel JSON files when present.
 * @returns {Promise<Map<string, Array>>}
 */
export async function loadMgNeighborPanelMap(projectPath, px, py, centerGrid) {
  const panelMap = new Map()
  panelMap.set(`${px},${py}`, centerGrid)
  const tasks = []
  for (let dy = -1; dy <= 1; dy++) {
    for (let dx = -1; dx <= 1; dx++) {
      ...
      const ny = py + dy
      tasks.push(
        window.electronAPI.loadPanelData(projectPath, nx, ny).then((res) => {
          if (res.success) panelMap.set(`${nx},${ny}`, res.data.grid)
        })
      )
    }
  }
...
function assignDerivedMapGeneratorLayers(
  ...
  if (t === T.ROAD) {
    ...
    const cliffExport = c.tileIndex
    // Composite cliffs: underlay should use the *high-side (uphill)* biome, then cliff overlay.
    // LayoutGen can mutate `c.biome` via post-passes, so derive both sides from neighbors when possible.
    let biomeHigh = c.biome ?? 0
    let biomeLow = c.biome ?? 0
    if (panelMap && worldX0 != null && worldY0 != null) {
      const [dx, dy] = cliffDownhillOffset(cliffExport)
      const wx = worldX0 + (i % PANEL)
      const wy = worldY0 + ((i / PANEL) | 0)
      const nbDown = mgWorldCell(panelMap, wx + dx, wy + dy)
      const nbUp = mgWorldCell(panelMap, wx - dx, wy - dy)
      if (nbUp && nbUp.biome != null) biomeHigh = nbUp.biome
      if (nbDown && nbDown.biome != null) biomeLow = nbDown.biome
    }
    c.layer1 = { type: T.CLIFF, tileIndex: cliffExport, biomeHigh, biomeLow }
  } else {
    c.tileIndex = 15
  }
}
...
export function recomputeMapGeneratorTileIndices(grid, context) {
  ...
  if (!context?.panelMap || context.px === undefined || context.py === undefined) {
    ...
    for (let i = 0; i < sz; i++) {
      const t = grid[i]?.type
      if (t === 'LAND' || t === 'FOREST' || t === 'ROAD' || t === 'CLIFF' || t === 'GRASS') landBmp[i] = 1
...
export async function mgBuildRenderProjectBundle({ project, mapping, assets, defaultAssets, onProgress }) {
  const cellPx = MG_FULL_EXPORT_CELL_PX
  const prepared = await mgPrepareMosaicData({
    project,
    mapping,
  ...
  const packTotal = compositionKeysSorted.length + 1
  let packCur = 0
  ...
  for (let si = 0; si < compositionKeysSorted.length; si++) {
    ...
    // Only cliff (hi,lo,ti) combos that appear on panels in this group — not the whole world.
    // Otherwise every atlas bakes every biome’s ground into composites “just in case,” which
    // fills single-biome tilesets with art from other biomes.
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!biomePack) continue
    const biomeComposition = compositionKey.split('_').map((n) => Number(n) | 0)
    const distinctBiomeCount = biomeComposition.length
    const rmLookup = biomePack.lookup
    const res = await window.electronAPI.loadPanelData(project.path, p.x, p.y)
    ...
    panels.push({
      ...
      biomeComposition,
      distinctBiomeCount,
      biomeTilesetIndex: biomePack.biomeTilesetIndex,
