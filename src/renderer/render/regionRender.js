...
// Double-height semantic indices: SE out(3), S(4), SW out(5), NW in(8), NE in(9)

const CLIFF_DBG_COLORS = [
  '#FF3333','#33FF57','#3399FF','#FF9900','#CC33FF',
  '#00FFFF','#FF33CC','#FFFF00','#FF6699','#00CC88',
  '#FF99FF','#CC6600','#CCFF33',
]
...
export function bakeRegion(region, exportMode = false, showPanelGrid = true, showBonusAreas = true) {
  ...
  // --- CLIFF_DOUBLE debug overlay: color-coded by tileIndex + legend ---
  const cliffDoubleDebug = document.createElement('canvas')
  cliffDoubleDebug.width = totalW
  cliffDoubleDebug.height = totalH
  const ddctx = cliffDoubleDebug.getContext('2d')
  ddctx.clearRect(0, 0, totalW, totalH)
  const cliffDoubleUsedIndices = new Set()
  if (!exportMode) {
    for (let py = 0; py < height; py++) {
      for (let px = 0; px < width; px++) {
        const pd = panelData[`${px},${py}`]
        if (!pd || !(pd.isRoute || pd.settlement)) continue
        for (let cy = 0; cy < PANEL; cy++) {
          for (let cx = 0; cx < PANEL; cx++) {
            const cell = pd.grid[cy * PANEL + cx]
            if (!cell) continue
            const gx = px * PANEL + cx
            const gy = py * PANEL + cy
            if (cell.type === T.CLIFF_DOUBLE && cell.tileIndex !== undefined) {
              cliffDoubleUsedIndices.add(cell.tileIndex)
              const baseBiome = (cell.biome !== undefined) ? cell.biome : pd.biome
              const pal = (baseBiome !== undefined && BIOME_PALETTES[baseBiome]) ? BIOME_PALETTES[baseBiome] : PALETTE
              const [r, g, b] = pal[T.CLIFF_DOUBLE]
              ddctx.fillStyle = `rgb(${r},${g},${b})`
              ddctx.fillRect(gx, gy, 1, 1)
            } else if (cell.type === T.CLIFF_DOUBLE_PART2) {
              const baseBiome = (cell.biome !== undefined) ? cell.biome : pd.biome
              const pal = (baseBiome !== undefined && BIOME_PALETTES[baseBiome]) ? BIOME_PALETTES[baseBiome] : PALETTE
              const [r, g, b] = pal[T.CLIFF_DOUBLE_PART2]
              ddctx.fillStyle = `rgb(${r},${g},${b})`
              ddctx.fillRect(gx, gy, 1, 1)
            }
          }
  ...
  return { map: offscreen, overlay, biomeMap, cliffBiomeMap, roadDebug, pocketOutlines, clusterOutlines, cliffDoubleDebug, cliffDoubleUsedIndices, CLIFF_DBG_COLORS }
}
...
export function renderRegion(
  ...
  if (baked) {
    ...
    if (baked.cliffDoubleDebug) {
      ctx.drawImage(baked.cliffDoubleDebug, offX, offY, renderW, renderH)
      // Legend hidden — CLIFF_DOUBLE = dark brown, PART2 = darker brown
    }
