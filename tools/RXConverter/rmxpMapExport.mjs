 ...
 * @param {string} [opts.regionName] - If provided, a top-level blank folder map is written first and all group parents are nested under it.
...
export function exportRenderBundleToRmxpDataDir(opts) {
  try {
    ...
    const regionName = opts?.regionName && String(opts.regionName).trim() || null
    if (startMapId < 2) {
      return {
        ok: false,
    ...
    // Optional top-level region folder — all group parents nest under it.
    let regionRootId = 0
    if (regionName) {
      regionRootId = nextMapId++
      const regionFile = writeBlankGroupParentRxdata(templateBuf, dataDir, regionRootId, tilesetId)
      written.push({
        mapId: regionRootId,
        file: regionFile,
        mapName: regionName,
        parentMapId: 0,
        rmxpRole: 'region_root',
      })
    }
